...
lcore_main()
{
    ...
    // pthread_mutex_unlock(&window_info_mutex);
    printf("test1\n");
    printf("windlow_len: %u\n", window_len);
    int i = 0;
    int seq_num = win_left + i;
    if (seq_num < NUM_PING)
    {
        printf("test - seq_num: %d\n", seq_num);
        // pthread_mutex_lock(&window_info_mutex);
        printf("seq_num: %d, sent_time: %" PRIu64 ", ack: %d\n", seq_num, window_sent_time[0][i], window_ack_mask[0][i]);
        if (window_sent_time[0][i] == 0 || (!window_ack_mask[0][i] && time_now(window_sent_time[0][i]) > (uint64_t)1e10))
        {
            printf("test2\n");
            // send a packet
    ...
    //             printf("test - seq_num: %d\n", seq_num);
    //             pthread_mutex_lock(&window_info_mutex);
    //             if (window_sent_time[i] == 0 || (!window_ack_mask[i] && time_now(window_sent_time[i]) > (uint64_t)1e9)) {
    //                 printf("test2\n");
    //                 // send a packet
    //                 pkt = rte_pktmbuf_alloc(mbuf_pool);
    //                 if (pkt == NULL) {
